...
class FsApi {
  ...
  // Poll all 10 status nodes in ONE session.
  // If the session is stolen mid-poll, logs and retries once with a fresh session.
  // Individual non-session-related node failures return null without affecting other nodes.
  async pollAll() {
    const runPoll = async (sid) => {
      let sessionStolen = false;
      const safeGet = async (node) => {
        try {
          return await this._doGet(sid, node);
        } catch (err) {
      ...
      const [power, volume, mute, mode, preset, playStatus, name, artist, text, album] = await Promise.all([
        safeGet('netRemote.sys.power'),
        safeGet('netRemote.sys.audio.volume'),
        safeGet('netRemote.sys.audio.mute'),
        safeGet('netRemote.sys.mode'),
        safeGet('netRemote.nav.action.selectPreset'),
        safeGet('netRemote.play.status'),
        safeGet('netRemote.play.info.name'),
        safeGet('netRemote.play.info.artist'),
        safeGet('netRemote.play.info.text'),
        safeGet('netRemote.play.info.album'),
      ]);

      if (sessionStolen) throw new SessionStolenError('session stolen during pollAll');
      return { power, volume, mute, mode, preset, playStatus, name, artist, text, album };
    };

    const sid = await this.createSession();
    try {
      return await runPoll(sid);
    } catch (err) {
      if (err instanceof SessionStolenError) {
        const sid2 = await this.createSession();
        return await runPoll(sid2);
      }
      throw err;
    }
  }
